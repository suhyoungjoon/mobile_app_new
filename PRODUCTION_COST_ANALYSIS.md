...
- **비용**: $0.7/월 (약 950원)
- **데이터 보관**: 무제한
- **용량**: 제한 있음 (정확한 용량은 플랜에 따라 다름)
- **백업**: 자동 백업 (기간은 플랜에 따라 다름)
- **연결 수**: 제한 있음 (플랜에 따라 다름)

> **참고**: 현재 Render PostgreSQL은 $0.7/월 플랜으로 업그레이드되어 있습니다. 이는 무료 플랜에서 한 단계 업그레이드된 플랜입니다.

#### 이전 권장: Starter 플랜 ($7/월)
- **비용**: $7/월 (약 9,500원)
- **데이터 보관**: 무제한
- **용량**: 1GB (필요 시 확장 가능)
- **자동 백업**: 7일간 보관
...
> **참고**: 현재 $0.7/월 플랜이 적용되어 있어 Starter 플랜보다 경제적입니다.
...
### 현재 구성 (실제 운영 중)
**약 11,100원-64,600원/월** (약 $8.20-$47.70/월)
- Vercel 플랜에 따라 비용 차이
- AI 판정 사용 여부에 따라 비용 차이

### 최소 구성 (기본 운영)
**약 65,200원/월** (약 $48.30/월)
- Render PostgreSQL $0.7/월 플랜 적용
- 파일 스토리지 S3 사용
...
- **데이터베이스**: Render PostgreSQL ($0.7/월) ✅ **업그레이드 완료**
- **파일 스토리지**: Render 디스크 (백엔드 포함) ($0/월)
- **AI 판정**: Azure OpenAI (선택사항) ($0-$20/월)
- **총 비용**: 약 $8.20-$47.70/월 (약 11,100원-64,600원)

### 비용 절감 효과
- **Render PostgreSQL**: $7/월 → $0.7/월로 절감 (약 $6.30/월, 8,500원/월 절감)
- **연간 절감액**: 약 $75.60/년 (약 102,000원/년)
...
### 현재 상태 요약
✅ **Render PostgreSQL $0.7/월 플랜으로 업그레이드 완료**  
