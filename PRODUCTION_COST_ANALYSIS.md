...
- Vercel 플랜에 따라 비용 차이 ($0-$20)
- AI 판정 설정에 따라 비용 차이 ($0-$20)
  - 최소: 로컬+Hugging Face 하이브리드 ($0/월)
  - 최대: Azure OpenAI만 사용 ($20/월)
- Render PostgreSQL Starter 플랜 ($7/월) 적용
- ✅ 푸시 알림: 무료 ($0/월)
- ✅ 개인정보 암호화: 무료 ($0/월)

### 최소 구성 (기본 운영) - 비용 절감 모드
**약 46,800원/월** (약 $34.60/월)
- Render PostgreSQL Starter 플랜 ($7/월) 적용
- 파일 스토리지 S3 사용 ($0.10/월)
- AI 판정: 하이브리드 모드 (로컬+Hugging Face) ($0/월)
- ✅ 모든 추가 기능 포함 (푸시 알림, 암호화 등)

### 권장 구성 (안정적 운영)
**약 115,600원/월** (약 $86/월)
- Professional 플랜 적용
- Azure OpenAI 사용 ($20/월)
- 파일 스토리지 S3 사용

### 연간 비용
- **현재 구성**: 약 235,200원-883,200원/년
- **최소 구성 (절감 모드)**: 약 561,600원/년
- **권장 구성**: 약 1,387,200원/년

### 비용 절감 효과
- **하이브리드 AI 모드 사용 시**: $20/월 → $0-$10/월 (약 50-100% 절감)
- **푸시 알림 사용**: SMS 대비 $0.50/월 절감 (월 100회 기준)
- **로컬 암호화**: 클라우드 암호화 서비스 대비 $0/월 (추가 비용 없음)

---

## 🎯 결론

### 현재 운영 구성 ✅ 모든 기능 구현 완료
...
- **AI 판정**: 다중 엔진 지원 ✅ **구현 완료**
  - 로컬 규칙 기반: $0/월
  - Hugging Face: $0/월 (무료 할당량 내)
  - Azure OpenAI: $0-$20/월 (선택사항)
  - 하이브리드 모드: $0-$10/월 (비용 절감)
- **푸시 알림**: VAPID Web Push ✅ **구현 완료** ($0/월)
- **개인정보 암호화**: AES-256-CBC ✅ **구현 완료** ($0/월)
- **YouTube 검색**: Google API ($0/월, 무료 할당량)
...
✅ **AI 판정 다중 엔진 지원 구현 완료** (로컬/Hugging Face/Azure/하이브리드)  
✅ **푸시 알림 시스템 구현 완료** (VAPID Web Push, 완전 무료)  
✅ **개인정보 암호화 구현 완료** (AES-256-CBC, 추가 비용 없음)  
✅ **YouTube 실시간 검색 구현 완료** (무료 할당량 내)  
✅ **안정적인 서비스 운영 구성 완료**  

### 비용 절감 핵심 포인트
1. **하이브리드 AI 모드**: 로컬+Hugging Face 조합으로 $0/월 운영 가능
2. **푸시 알림**: SMS 대비 $0.50/월 절감 (Web Push API 무료)
3. **로컬 암호화**: 클라우드 암호화 서비스 대비 추가 비용 없음
4. **최소 구성 비용**: 약 $34.60/월 (약 46,800원/월)로 운영 가능
